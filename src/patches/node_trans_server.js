...
class NodeTransServer {
  ...
  async run() {
    ...
    try {
      ...
      Logger.error(
        `Node Media Trans Server startup failed. ffmpeg:${this.config.trans.ffmpeg} cannot be executed.`
      );
      return;
    }
    ...
    try {
      ...
      Logger.warn("无法获取FFmpeg版本信息");
    }

    Logger.log(
      `Node Media Trans Server started for apps: [ ${apps}] , MediaRoot: ${this.config.http.mediaroot}, ffmpeg version: ${ffmpegVersion}`
    );
  }
